...
public class EmployeeAdd extends HttpServlet {
	...
	protected void doPost(
		...
		try {
			...
			if (count > 0) {
				// sendRedirect(url) 메서드 호출 전에 출력된 모든 것들은 무시된다.
				// => 리다이렉트는 클라이언트에 결과 내용을 보내지 않는다.
				// => 응답 헤더만 보낸다.
				// => url은 상대경로 지정 가능하다.
				...
				/* sendRedirect() 실행 결과 
				 * 응답 상태 302와 Location 헤더가 리다이렉트 결과이다.
				 */
				/*
				 	HTTP/1.1 302 Found
					Server: Apache-Coyote/1.1
					Location: http://localhost:9999/web01/hr/searchEmp
					Content-Type: text/html;charset=UTF-8
					Content-Length: 0
					Date: Mon, 03 Jun 2013 05:10:50 GMT
					Expires: 0
